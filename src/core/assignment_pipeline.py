...
class AssignmentPipeline:
    ...
    def run_clustering(self) -> pd.DataFrame:
        ...
        assert self._cached_tfidf_matrix is not None

        clustering_result = self.clusterer.run_clustering(
            tfidf_matrix=self._cached_tfidf_matrix,
    ...
    def run_anomaly_detection(self) -> pd.DataFrame:
        ...
        anomaly_mask, anomaly_scores = self.anomaly_detector.run_detection(self._cached_tfidf_matrix)
    ...
    def _ensure_features_ready(self) -> None:
        """Loads data and creates TF-IDF features only once."""
        ...
        if self._cached_tfidf_matrix is None:
            self._cached_tfidf_matrix = self.preprocessor.fit_transform(
